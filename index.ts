...
class OfflineMirrorRegistry {
  ...
  constructor(props: OfflineMirrorRegistryProps) {
    this.cache = props.cache || new NodeCache();
  ...
  public async getPackages(
    ...
    const matched = glob.sync(`${filePrefix}-+([0-9]).+([0-9]).+([0-9]).tgz`, {
      cwd: dir,
      nodir: true,
      absolute: true,
    });
  ...
  public getServer(): express.Express {
    ...
    app.get("/:package([^/]+((/[^/]+)?))", async (req, res) => {
      ...
      const packagesRecord = await this.getPackages(this.directory, pkg);
      if (!packagesRecord) {
        // 404
        res.status(404).json([]);
        return;
    ...
    app.get("/:package([^/]+((/[^/]+)?))/-/:tarball", async (req, res) => {
      ...
      const cachedPath = this.cache.get(`tar:${pkg}:${tarball}`);
      if (!cachedPath) {
